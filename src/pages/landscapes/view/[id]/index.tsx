...
import { getLandscapeById } from 'apiSdk/landscapes';
import { LandscapeInterface } from 'interfaces/landscape';

function LandscapeViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<LandscapeInterface>(
    () => (id ? `/landscapes/${id}` : null),
    () =>
      getLandscapeById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('landscape', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/landscapes/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Landscape Status" text={data?.landscape_status} />

                <FormListItem label="Landscape Score" text={data?.landscape_score} />
