...
import { getSecurities } from 'apiSdk/securities';
import { SecurityInterface } from 'interfaces/security';
import { getLandscapes } from 'apiSdk/landscapes';
import { LandscapeInterface } from 'interfaces/landscape';
import { getConfigurations } from 'apiSdk/configurations';
import { ConfigurationInterface } from 'interfaces/configuration';

import { DataPrivacyListPage as ShowCaseList } from 'pages/data-privacies';

function DashboardPage() {
  const { session } = useSession();

  const securityStat = useSWR<PaginatedInterface<SecurityInterface>>(
    () => `/securities`,
    () => getSecurities({ limit: 1, offset: 0 }),
  );

  const landscapeStat = useSWR<PaginatedInterface<LandscapeInterface>>(
    () => `/landscapes`,
    () => getLandscapes({ limit: 1, offset: 0 }),
  );

  const configurationStat = useSWR<PaginatedInterface<ConfigurationInterface>>(
    () => `/configurations`,
    () => getConfigurations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Securities',
      result: securityStat?.data?.totalCount,
      isLoading: securityStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Landscapes',
      result: landscapeStat?.data?.totalCount,
      isLoading: landscapeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Configurations',
      result: configurationStat?.data?.totalCount,
      isLoading: configurationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Data Privacies
                </Text>
                <NextLink href={`data-privacies`} passHref legacyBehavior>
                  <Button
